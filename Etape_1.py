class MiseFondsInitiale:
    ...
    def __init__(self):
       ...
       self.conversion_taux = input("Le taux d'intérêts donné est-il en année, "
                                    "en trimestre, en mois, en semaine ou en jour? ").strip().lower()
       self.taux_global = (float(input("Quel est le taux d'intérêts du projet en pourcentage? "))) / 100
       self.mise_initiale = float(input("Quel est le montant de cette mise de fonds? ").replace(" ", ""))
       self.mise_temps = int(input("Quelle est l'année du projet correspondant "
                                   "à cette mise de fonds? (le projet débute en l'an 0) "))
    ...
    def mise_de_fonds_initiale(self):
        mise = self.mise_initiale * ((1 + self.taux_global) ** (-1 * self.mise_temps))
        mise = -1 * mise
        return mise
